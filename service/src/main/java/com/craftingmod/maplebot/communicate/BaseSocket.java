...
public abstract class BaseSocket extends BroadcastReceiver {
    ...
    public static BaseSocket buildClient(final Context context, final String name,final int timeoutS,final int timeoutR){
        BaseSocket base = new BaseSocket(context) {
            @Override
            protected String getName() {
                return name;
            ...
            @Override
            protected int getTimeout(boolean isStatus) {
                return (isStatus)?timeoutS:timeoutR;
            }
        };
        base.registerBroadcast(context);
        return base;
    ...
    public void request(String id,String data,NextTask<ResultObject> task){
        ...
        timer.schedule(timeout,getTimeout(false));
        context.sendBroadcast(intent);
    }
